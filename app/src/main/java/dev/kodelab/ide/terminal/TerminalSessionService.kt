...
class TerminalSessionService : Service() {
    ...
    /** Open session ids, in the order they were created — the terminal's tabs. */
    private val _sessionIds = kotlinx.coroutines.flow.MutableStateFlow<List<String>>(emptyList())
    val sessionIds: kotlinx.coroutines.flow.StateFlow<List<String>> = _sessionIds
    ...
    fun openSession(id: String, cwd: String?): ShellSession =
        sessions.getOrPut(id) {
            ShellSession(id, cwd, sandbox).also { it.start() }
        }.also { _sessionIds.value = sessions.keys.toList() }
    ...
    /** A brand-new session alongside the existing ones (the terminal's "+"). */
    fun newSession(cwd: String?): ShellSession {
        val n = generateSequence(1) { it + 1 }.first { "term-$it" !in sessions }
        return openSession("term-$n", cwd)
    }
